...
class BlockchainController {
    ...
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            let validateErrors = [];
            try {
                validateErrors = await this.blockchain.validateChain();
            } catch (error) {
                console.log(error);
                return res.status(500).send("An error ocurred.");
            }
            if(validateErrors.length != 0){
                return res.status(200).json("Validate errors: " + validateErrors);
            } else {
                return res.status(200).json("No errors found at validation.");
            }
